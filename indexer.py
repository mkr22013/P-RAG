...
def classify_document(md_content):
    ...
    try:
        ...
        print(f"[*] Classifying document using snippet ({len(header_snippet)} chars)...")
        ...
        return {
            "year": int(clean_year) if clean_year else 2026, # Default to 2026 if extraction fails
...
def generate_sub_index(md_content, sub_index_path):
    ...
    # 1. FORCED CHUNKING
    chunk_size = 4000
    final_chunks = [md_content[i:i+chunk_size] for i in range(0, len(md_content), chunk_size)]
    ...
    print(f"[*] Indexing {total} sections ({'FAST MODE' if is_massive else 'AI MODE'})...")
    ...
    for i, chunk in enumerate(final_chunks):
        ...
        if is_massive:
            ...
            keywords = list(set(re.findall(r'\b\w{5,}\b', chunk.lower())))[:5] # 5+ letter words
        else:
            ...
            try:
                ...
                response = ollama.generate(model=LOCAL_MODEL, prompt=prompt, format="json", options={"num_ctx": 4096})
...
def build_all():
    ...
    for root, _, files in os.walk(DOC_BASE_DIR):
        for filename in files:
            ...
            cursor.execute("SELECT sub_index_path FROM master_index WHERE pdf_path = ?", (pdf_path,))
            existing = cursor.fetchone()
            if existing:
                print(f"[*] Overwriting existing index for: {filename}")
                if os.path.exists(existing[0]): os.remove(existing[0])
                cursor.execute("DELETE FROM master_index WHERE pdf_path = ?", (pdf_path,))

            try:
                print(f"[*] Docling parsing: {filename} (Complex files may take 2+ mins)...")
                ...
                if plan_info and plan_info['year'] != 0:
                    fn = f"{plan_info['year']}_{plan_info['type']}_{plan_info['tier']}.json".lower().replace(" ","_")
                    ...
                    cursor.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    ''', (plan_info['year'], plan_info['type'], plan_info['tier'], pdf_path, sub_index_path))
                    print(f"✅ SUCCESS: {plan_info['year']} {plan_info['tier']} {plan_info['type']}")
