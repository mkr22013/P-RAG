...
class Settings:
    ...
    # ── LLM Provider ─────────────────────────────────────────────────────────
    # "ollama"  → local Ollama (dev default)
    # "openai"  → any OpenAI-compatible gateway (Portkey, Azure OpenAI, LiteLLM etc.)
    LLM_PROVIDER: str = os.getenv("LLM_PROVIDER", "ollama")

    # Ollama — local dev only
    OLLAMA_MODEL: str = os.getenv("OLLAMA_MODEL", "llama3.1")

    # OpenAI-compatible gateway — used when LLM_PROVIDER=openai
    # Works for: Portkey, Azure OpenAI direct, LiteLLM, any OpenAI-compatible API
    LLM_BASE_URL: str = os.getenv("LLM_BASE_URL", "")  # e.g. https://api.portkey.ai/v1
    LLM_API_KEY: str = os.getenv("LLM_API_KEY", "")  # * Key Vault
    LLM_MODEL: str = os.getenv("LLM_MODEL", "gpt-4o-mini")  # model name for the gateway
    LLM_VIRTUAL_KEY: str = os.getenv(
        "LLM_VIRTUAL_KEY", ""
...
# # ==============================================Previous configuration before adding portkey changes=========================================================
# # """
# # config.py
# # ────────────────────────────────────────────────────────────────────────────
# # Environment-aware configuration loader — Python equivalent of .NET Program.cs
