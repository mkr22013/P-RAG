...
def verify_system_integrity(db_path="insurance_index.db"):
    ...
    try:
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='master_index'"
        )
        ...
        if not rows:
            ...
            print(
                f"{'ID':<4} | {'Plan Details':<25} | {'PDF Status':<10} | {'Sub-Index Status'}"
            )
            print("-" * 80)
            ...
            for row in rows:
                ...
                print(
                    f"{doc_id:<4} | {plan_label:<25} | {pdf_status:<10} | {idx_status}"
                )
